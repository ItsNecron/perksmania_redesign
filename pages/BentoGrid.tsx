    ...
    <section id="perks-grid" className="py-12 bg-white scroll-mt-24">
      <div className="container mx-auto px-4 lg:px-8">
        ...
        <div className="flex flex-col md:flex-row items-end justify-between mb-8 gap-4">
          <div>
            <h2 className="text-3xl font-extrabold text-brand-primary flex items-center gap-2">
            ...
            <p className="text-slate-500 mt-1">
              {searchQuery
                ...
                : "Don't miss out on what everyone is claiming."}
            </p>
          </div>
          <button
            onClick={handleReset}
        ...
        {isEmpty ? (
          <div className="py-20 flex flex-col items-center justify-center text-center">
            ...
            <button
              onClick={handleReset}
              className="px-6 py-2 bg-brand-secondary text-white rounded-lg font-bold hover:bg-brand-primary transition-colors"
            >
              Clear Filters
            </button>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
            {filteredPopular.map((perk, index) => (
              ...
              <div
                key={perk.id}
                className={
                  !activeCategory && !searchQuery && index === 0
                    ? "md:col-span-2"
                    : ""
                }
              >
                <ProductCard
                  product={perk}
                  featured={!activeCategory && !searchQuery && index === 0}
                />
              </div>
            ))}
          </div>
              ...
              {!activeCategory && !searchQuery && (
                <div className="rounded-3xl bg-gradient-to-br from-purple-600 to-brand-secondary p-5 mob-m:p-6 lg:p-8 text-white flex flex-col justify-center items-start shadow-xl">
                  ...
                  <p className="text-white/80 mb-6 text-sm">
                    Join the VIP list to get early access to drops.
                  </p>
                  <form
                    onSubmit={handleSubscribe}
                    className="w-full bg-white/20 p-1 rounded-xl flex"
                  >
                    <input
                      type="email"
                      value={email}
                    ...
                    <button
                      type="submit"
                      className="bg-white text-brand-secondary rounded-lg px-4 py-2 text-sm font-bold shadow-md hover:bg-slate-100 transition-colors"
                    >
                      Go
                    </button>
                  </form>
